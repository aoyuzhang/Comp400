...
public class Convolution extends Thread {
  ...
  public static double singlePixelConvolution(double [][] input, 
    ...
    int upperLeftConerX=x-kernelWidth/2;
    int upperLeftCornerY= y-kernelHeight/2;
    for(int i=0;i<kernelWidth;++i)
    {
      for(int j=0;j<kernelHeight;++j)
      {
    	  if(i<0||j<0||i>=input[0].length||j>=input.length)
    		  continue;
    	  output = output + (input[upperLeftConerX+i][upperLeftCornerY+j] * k[i][j]);
      }
    }
    return output;
  }
